...
/**
 * Rate limit: 3 submissions per IP per 10 minutes, per server instance.
 *
 * In-memory, so it resets on restart and is not shared between instances.
 * Adequate against casual spam; move to a shared store (Upstash/Redis) if the
 * form starts attracting real abuse.
 */
...
export async function POST(request: Request) {
  try {
    ...
    // Only the friendly number is returned — never the internal uuid.
